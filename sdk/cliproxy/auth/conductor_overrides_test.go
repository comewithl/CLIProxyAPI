...
func TestManager_MarkResult_Transient5xxDoesNotBlockModel(t *testing.T) {
	m := NewManager(nil, nil, nil)
	...
	m.MarkResult(context.Background(), Result{
		...
		Model:    model,
		Success:  false,
		Error:    &Error{HTTPStatus: 500, Message: "upstream overload"},
	})
	...
	if !state.NextRetryAfter.IsZero() {
		t.Fatalf("expected NextRetryAfter to be zero for transient 5xx, got %v", state.NextRetryAfter)
	}

	blocked, _, _ := isAuthBlockedForModel(updated, model, time.Now())
	if blocked {
		t.Fatalf("expected auth to remain selectable for model after transient 5xx")
	}
}
